...
- **Test ↔ src mapping:** `test/copy.jl` ↔ `src/copy.jl`; `test/sgp4.jl` ↔ `src/sgp4_model.jl`; `test/tle.jl` ↔ `src/fit.jl` (TLE sink); `test/omm.jl` ↔ `src/omm.jl` and `src/fit.jl` (OMM sink); `test/quality.jl` and `test/performance.jl` cover the whole package. `src/types.jl` and `src/precompile.jl` have no dedicated test file (covered indirectly). Test fixtures live in `test/sgp4_tests/` (AIAA 2006-6753 reference TLEs and expected results) and `test/omm_tests/` (a public Space-Track OMM in XML).
