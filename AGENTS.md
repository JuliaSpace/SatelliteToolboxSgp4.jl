...
- **Module entrypoint:** `src/SatelliteToolboxSgp4.jl`. Include order is fixed and load-order-sensitive: `types.jl` → `copy.jl` → `show.jl` → `sgp4_model.jl` → `tle.jl` → `omm.jl` → `fit.jl` → `precompile.jl`. `fit.jl` holds the least-square fitting (`fit_sgp4_mean_elements[!]`, `update_sgp4_mean_elements_epoch[!]`) over a format-agnostic mean state vector; `tle.jl` and `omm.jl` only provide the conversions (`_mean_state_vector`, `_mean_elements_epoch`, `_build_mean_elements(::Type{S}, ...)`) for their format. The output type is selected by the sink type `S <: Union{TLE, OrbitMeanElementsMessage}` passed as the first argument (second for the `!` form, after the propagator); without a sink type, the output is an `OrbitMeanElementsMessage`. The fitting returns `(mean_elements, covariance, stats)`, where `stats` is a `NamedTuple` (`converged`, `iterations`, `position_rmse`, `velocity_rmse`, `total_rmse`); the epoch update returns only the mean elements. New code must respect this order; symbols defined in an earlier file are visible to later ones, not vice versa.
- **Number type rule:** the propagator number type `T` is always the number type of the `Sgp4Constants` passed as `sgp4c` (`SGP4C_WGS84`/`SGP4C_WGS72`, both `Float64`); inputs are converted to `T`. Other types (e.g. `Float32`, ForwardDiff duals) are selected with `Sgp4Constants{T}(sgp4c)`. There are no `_f32` constants. `Sgp4DeepSpace` is immutable and stored inline in `Sgp4Propagator`; the resonance integrator state lives in `Sgp4Propagator.{atime, xli, xni}`. Create propagators for `sgp4_init!` with `Sgp4Propagator{Tepoch}(sgp4c)` or `Sgp4Propagator(sgp4c)` (`Float64` epoch). `SatelliteToolboxSgp4.is_initialized(sgp4d)` is public but not exported.
...
- **Test ↔ src mapping:** `test/copy.jl` ↔ `src/copy.jl`; `test/sgp4.jl` ↔ `src/sgp4_model.jl`; `test/tle.jl` ↔ `src/fit.jl` (TLE sink); `test/omm.jl` ↔ `src/omm.jl` and `src/fit.jl` (OMM sink); `test/quality.jl` and `test/performance.jl` cover the whole package. `src/show.jl` is tested in `test/sgp4.jl` ("Show"); `src/types.jl` and `src/precompile.jl` have no dedicated test file (covered indirectly). Test fixtures live in `test/sgp4_tests/` (AIAA 2006-6753 reference TLEs and expected results) and `test/omm_tests/` (a public Space-Track OMM in XML).
...
- **Focused test file:** `julia --project=. -e 'using SatelliteToolboxSgp4, Test, Dates, DelimitedFiles, Printf, SatelliteToolboxTle; include("test/<file>.jl")'` — extend the `using` list with any extra deps the target file needs (e.g. `ForwardDiff, StaticArrays` for `performance.jl`, `SatelliteToolboxOrbitDataMessages` for `omm.jl`). Fixture paths are relative to `test/`, so `cd("test")` first (or run from that directory). There is no `@testitem`/test-name selector; `Pkg.test(test_args=...)` is not read by `runtests.jl`.
